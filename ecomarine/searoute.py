...
@cache
def setup_M():
    from .data.marnet_dict_eca_distance_with_remove_60_north import (
        edge_list as marnet_e,
    )
    from .data.marnet_dict_eca_distance_with_remove_60_north import (
        node_list as marnet_n,
    )
...
def searoute(
    ...
    if include_ports:
        ...
        closestPortDest = P.query(terminals=only_terminals, cty=country_pod).kdtree.query(
            destination
        )
        if closestPortDest:
    ...
    """
    ls, traversed_passages, eca_distance = process_route(
        shortest_route_by_distance, M, return_passages
    )

    total_length = distance_length(ls, units=units)

    converted_eca_dist_value, _ = convert_km_to_unit(distance_km=eca_distance, target_unit=units)
    ...
    feature = Feature(
        ...
        properties={
            ...
            "eca_distance": round(converted_eca_dist_value, 2),
        },
    )
    ...
    if return_passages:
        feature.properties["traversed_passages"] = passages.Passage.filter_valid_passages(
            traversed_passages
        )
