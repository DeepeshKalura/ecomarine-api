...
class Ports(nx.Graph):
    ...
    def query(self, terminals: bool = True, cty: str = None, to_cty: str = None, strict=False):
        ...
        filtered_nodes = [
            (n, data) for n, data in self.nodes(data=True) if data.get("t") == terminal_filter
        ]
        ...
        if len(filtered_nodes) > 0:
            ...
            filtered_nodes_prev = filtered_nodes
            filtered_nodes = [
                (n, data) for n, data in filtered_nodes_prev if cty_to_filter(data, to_cty)
            ]
    ...
    def get_preferred_ports(self, x, y, ft: FeatureCollection, top=None, include_area_name=False):
        ...
        if include_area_name:
            ...
            sorted_by_second = sorted(
                [(a, b / max(_sum, 1), _update_props(a, c)) for a, b, c in preferred_ports],
                key=lambda tup: tup[1],
                reverse=True,
            )
