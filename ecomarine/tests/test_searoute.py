...
def test_rev_lat_lon_passages():
    traj = ec.searoute(
        [140.02, 35.51], [-97.36, 27.81], append_orig_dest=True, return_passages=True
    )
    traversed = traj["properties"]["traversed_passages"]
    assert "suez" in traversed
