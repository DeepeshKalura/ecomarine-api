...
class TestPortProps(unittest.TestCase):
    ...
    def test_preferred_ports(self):
        ...
        # europan area
        eurArea = AreaFeature(
            get_eur_like_poly(), name="EUR", preferred_ports=[first_port, second_port]
        )

        result = ec.setup_P().get_preferred_ports(
            *point, AreaFeature.create([eurArea]), top=None, include_area_name=True
        )
        ...
        chArea = AreaFeature(get_suisse_poly(), name="CH", preferred_ports=[first_port])

        result = ec.setup_P().get_preferred_ports(
            *point,
            AreaFeature.create([eurArea, chArea]),
            top=None,
