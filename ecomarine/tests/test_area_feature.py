...
from ecomarine.classes.area_feature import AreaFeature
from ecomarine.classes.ports_props import PortProps
from ecomarine.tests.test_utils import (
...
def test_norm_preferred_ports():
    area = AreaFeature(
        coordinates=geojson.utils.generate_random("Polygon").coordinates,
...
def test_contains():
    ...
    assert areaCH.contains(*basel_point) == True

    # test if Paris is not in Switweland
    paris_point = (2.333333, 48.866667)  # Paris
    assert areaCH.contains(*paris_point) == False
    ...
    # Paris in EUR
    assert areaEUR.contains(*paris_point) == True

    # Basel in EUR
    assert areaEUR.contains(*basel_point) == True

    # first 10 point of CH in EUR
    for ch_p in areaCH.geometry.coordinates[0][:10]:
        assert areaEUR.contains(*ch_p) == True

    # New york is not in EUR
    new_york_point = (-74.005941, 40.712784)
    assert areaEUR.contains(*new_york_point) == False

    # Tokyo is not in EUR
    tokyo_point = (139.839478, 35.652832)
    assert areaEUR.contains(*tokyo_point) == False
