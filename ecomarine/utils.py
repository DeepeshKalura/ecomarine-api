...
def find_lowest_key(data, *keys):
    min_dict_ids = tuple(
        min(data, key=lambda dict_id: data[dict_id].get(key, float("inf"))) for key in keys
    )
...
def from_nodes_edges_set(G, node_set, edge_set):
    ...
    >>> M = oc.from_nodes_edges_set( oc.Marnet() ,my_nodes, my_edges)

    or

    >>> P = oc.from_nodes_edges_set( oc.Ports() ,my_nodes, None) # for Ports
    ...
    for gf in geojson_file:
        ...
        def handle_geometry(geometry, properties):
            if geometry.type == "LineString":
                ...
                for u, v in zip(coords[:-1], coords[1:], strict=False):
                ...
                for line_string in geometry.coordinates:
                    for u, v in zip(line_string[:-1], line_string[1:], strict=False):
                        G.add_edge(tuple(u), tuple(v), **properties)
                        # the other side as well
                        G.add_edge(tuple(v), tuple(u), **properties)
        ...
        if data.type == "FeatureCollection":
            G.graph["crs"] = (
                data.get("crs", {}).get("properties", {}).get("name", None) or G.graph["crs"]
            )
...
def process_route(shortest_route_by_distance, M, return_passages=False):
    """
    Process the shortest route by distance, normalizing coordinates,
    extracting passages, and summing ECA distance.
    """
    ls = []
    # `previous_coord_for_norm` for normalizing the visual LineString points
    previous_coord_for_norm = None

    total_eca_distance_km = 0.0
    traversed_passages_list = []

    if not shortest_route_by_distance or len(shortest_route_by_distance) < 2:
        return [], [], 0.0

    # Iterate over the rest of the path to form segments
    for i in range(len(shortest_route_by_distance) - 1):
        start_node = shortest_route_by_distance[i]
        end_node = shortest_route_by_distance[i + 1]

        # Normalize the start coordinate for the visual LineString
        if i == 0:
            normalized_start_coord = normalize_linestring(None, start_node)
            ls.append(normalized_start_coord)
            previous_coord_for_norm = normalized_start_coord

        # Normalize the end coordinate for the visual LineString
        normalized_end_coord = normalize_linestring(previous_coord_for_norm, end_node)
        ls.append(normalized_end_coord)
        previous_coord_for_norm = normalized_end_coord

        # Get edge data and sum the pre-calculated ECA distance
        edge = M.get_edge_data(start_node, end_node)
        if edge:
            # THIS IS THE KEY: Just add the pre-calculated value!
            total_eca_distance_km += edge.get("eca_distance_km", 0.0)

            if return_passages:
                passage = edge.get("passage")
                if passage:
                    traversed_passages_list.append(passage)

    final_passages = traversed_passages_list if return_passages else None

    return ls, final_passages, total_eca_distance_km
...
def convert_km_to_unit(distance_km: float, target_unit: str) -> tuple[float, str]:
    """
    Converts a distance from kilometers to a specified target unit.

    Args:
        distance_km (float): The distance in kilometers.
        target_unit (str): The desired output unit (e.g., "mi", "naut", "m").

    Returns:
        tuple[float, str]: A tuple containing the converted distance and the unit string.
                           If conversion is not possible for the target_unit,
                           returns the original distance_km and "km".
    """
    ...
    if target_unit in conversions:
        # 'conversions' dict provides factor from METERS to target_unit
        distance_meters = distance_km * 1000.0
        conversion_factor = conversions.get(
            target_unit
        )  # Should exist if target_unit in conversions

        if conversion_factor is not None:
            converted_distance = distance_meters * conversion_factor
            return converted_distance, target_unit
        else:
            # This case should ideally not be reached if target_unit in conversions check is robust
            # print(f"Warning: Conversion factor for unit '{target_unit}' is None. Returning km.")
            return distance_km, "km"
    else:
        # If the target_unit is not 'km' and not in the 'conversions' dictionary
        # print(f"Warning: Unknown target unit '{target_unit}' for conversion. Returning original distance in km.")
        return distance_km, "km"
